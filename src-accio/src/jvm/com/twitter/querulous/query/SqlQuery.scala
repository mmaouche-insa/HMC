...
class SqlQueryFactory extends QueryFactory {
  def apply(connection: Connection, queryClass: QueryClass, query: String, params: Any*): SqlQuery =
...
object NullValues {
  ...
  private val nullTypes =
    classOf[Types].getFields
      .filter(selectTypeValFields)
      .map { f: Field =>
   ...
   */
  def apply(typeVal: Int): NullValue = nullTypes(typeVal)
}

class SqlQuery(connection: Connection, val query: String, params: Any*) extends Query {
  def this(connection: Connection, query: String) = this(connection, query, Nil)
  ...
  private def expandArrayParams(query: String, params: Any*): String = {
    ...
    def marks(param: Any): String = param match {
      ...
      case t4: (_, _, _, _) => "(?,?,?,?)"
      case a: Array[Byte] => "?"
      case s: Iterable[_] => s.toSeq.map(marks).mkString(",")
      case _ => "?"
    }
